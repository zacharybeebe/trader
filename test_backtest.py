...
if __name__ == '__main__':
    ...
    fdf = format_data(data=df, inplace=True, column_mappers={'dt': 'date'})
    last_row = fdf.iloc[-1]
    print(f'{last_row=}')
    print(f"{last_row['close']=}\t{type(last_row['close'])=}")
    print(f'{last_row.index=}')
    ...
    # #     gain_pct=range(5, 11),
    # #     retract_pct=range(5, 11),
    # #     reset_low_pct=range(15, 25),
    # #     reset_high_pct=range(15, 25)
    # # )
    # # print(odf.to_string())
